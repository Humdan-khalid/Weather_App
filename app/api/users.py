...
from app.core.exceptions import UserAlreadyExist, ServerError, InvalidCredentials, DatabaseError
...
async def user_new_account(user:CreateUsers, session: AsyncSession=Depends(get_session)):
    try:
        ...
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="User account already exist.")
