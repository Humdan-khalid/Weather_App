...
async def get_user_history(session: AsyncSession, user: dict):
    ...
    cache = await caching.get_user_weather_history_from_cache(user['id'])
...
async def get_top_search_location(session: AsyncSession, admin: dict):
...
async def get_top_data_user(session: AsyncSession, admin):
    db_admin = await auth_repo.admin_authentication_with_email(session, admin['email'])
    ...
    db_user = await weather_repo.find_top_user(session)
